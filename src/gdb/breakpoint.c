		    ...
		    int *hw_breakpoint_error)
{
  ...
  if (bpt->loc_type == bp_loc_software_breakpoint
	...
	{
	  /* No overlay handling: just set the breakpoint. Pass core_number
	     information needed for multicore debugging.  */

	  if (bpt->loc_type == bp_loc_hardware_breakpoint)
	    val = target_insert_mc_hw_breakpoint (&bpt->target_info, bpt->core_number);
	  else
      ...
      else
	{
	  ...
	  if (section_is_mapped (bpt->section))
	    {
	      /* Yes.  This overlay section is mapped into memory. Pass 
	 	 core_number needed for multicore debugging.  */
	      if (bpt->loc_type == bp_loc_hardware_breakpoint)
		val = target_insert_mc_hw_breakpoint (&bpt->target_info, bpt->core_number);
	      else
		val = target_insert_breakpoint (&bpt->target_info);
...
reattach_breakpoints (int pid)
{
  ...
  ALL_BP_LOCATIONS (b)
  {
	...
	/* Pass core_number needed for multicore debugging.  */
	if (b->loc_type == bp_loc_hardware_breakpoint)
	  val = target_insert_mc_hw_breakpoint (&b->target_info, b->core_number);
	else
...
remove_breakpoint (struct bp_location *b, insertion_state_t is)
{
  ...
  if (b->loc_type == bp_loc_software_breakpoint
	...
	{
	  /* No overlay handling: just remove the breakpoint. Pass core_number 
	     needed for multicore debugging.  */

	  if (b->loc_type == bp_loc_hardware_breakpoint)
	    val = target_remove_mc_hw_breakpoint (&b->target_info, b->core_number);
	  else
	  ...
	  if (!overlay_events_enabled)
		...
		*/
		/* Ignore any failures: if the LMA is in ROM, we will
		   have already warned when we failed to insert it. Pass 
		   core_number needed for multicore debugging.  */
		if (b->loc_type == bp_loc_hardware_breakpoint)
	          target_remove_mc_hw_breakpoint (&b->overlay_target_info, b->core_number);
		else
	  ...
	  if (b->inserted)
	    {
		 ...
		 don't know what the overlay manager might do. Pass core_number
		 needed for multicore debugging.  */
	      if (b->loc_type == bp_loc_hardware_breakpoint)
	        val = target_remove_mc_hw_breakpoint (&b->target_info, b->core_number);
	      else
		      ...
		      CORE_ADDR *last_addr)
{
  ...
  /* Display the hardware breakpoints that are applied to the focused core.  */ 
  if (b->loc->core_number != -1
      && b->loc->core_number != target_get_core_number())
    return;
...
hw_breakpoint_used_count (void)
{
  ...
  ALL_BREAKPOINTS (b)
  {
	...
	/* Also ignore if breakpoint is core-specific but does not belong
	   to the current core.  */
        && (b->loc->core_number == -1
	    || b->loc->core_number == target_get_core_number ()))
      i++;
		    ...
		    struct breakpoint *pending_bp)
{
  ...
  /* Now set all the breakpoints.  */
  {
	...
	set_breakpoint_count (breakpoint_count + 1);
	b->number = breakpoint_count;
	/* If hardware breakpoint is core-specific record the core number.  */
	if (target_multicore_hw_breakpoint () && b->type == bp_hardware_breakpoint)
	  b->loc->core_number = target_get_core_number ();
	b->cond = cond[i];
	b->thread = thread;
	if (addr_string[i])
...
delete_breakpoint (struct breakpoint *bpt)
{
  ...
  if (bpt->loc->inserted
	    ...
	    && b->enable_state != bp_call_disabled)
	{
	  ...
	  /* Pass core_number, needed for multicore debugging.  */
	  if (b->type == bp_hardware_breakpoint)
	    val = target_insert_mc_hw_breakpoint (&b->loc->target_info, 
						  b->loc->core_number);
	  else
...
disable_breakpoint (struct breakpoint *bpt)
{
  ...
  /* Disable the hardware breakpoints that are applied to the focused core.  */
  if (bpt->loc->core_number != -1
      && target_get_core_number () != bpt->loc->core_number)
    {
      printf_filtered("\
Breakpoint not disabled, does not belong to the focused core.\n");
      return;
    }
...
do_enable_breakpoint (struct breakpoint *bpt, enum bpdisp disposition)
{
  ...
  if (bpt->type == bp_hardware_breakpoint)
    {
      int i;
      /* Enable the hardware breakpoints that are applied to the focused core. */
      if (bpt->loc->core_number != -1
	  && target_get_core_number () != bpt->loc->core_number)
	{
	  printf_filtered("\
Breakpoint is not enabled, does not belong to the focused core.\n");
	  return;
	}

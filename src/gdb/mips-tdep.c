			...
			gdb_byte *readbuf, const gdb_byte *writebuf)
{
  ...
  if (TYPE_LENGTH (type) <= register_size (gdbarch, regnum))
    {
  ...
  else
    {
	 ...
	 rules.  */
      ...
      offset = len - size;
      xfer = len - xfer;
      mips_xfer_register (regcache, NUM_REGS + regnum + 1, xfer,
                          TARGET_BYTE_ORDER, readbuf, writebuf, offset);
    }
...
mips_gdbarch_init (struct gdbarch_info info, struct gdbarch_list *arches)
{
  ...
  switch (mips_abi)
    ...
    case MIPS_ABI_EABI64:
      ...
      tdep->mips_last_fp_arg_regnum = tdep->regnum->fp0 + 12 + 8 - 1;
      tdep->default_mask_address_p = 0;
      /* The GCC-4.1.0 generates location list for symbols. These symbols
         are 4 bytes (32-bits) in EABI ABI. If addr_bits are not set then
         the debugger default to ptr_bits which is 64-bits. This value is
         used by dwarf2_read_address() function.  */

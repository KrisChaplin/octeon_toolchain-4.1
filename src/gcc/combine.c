		  ...
		  if ((set = single_set (temp)) != 0
			  ...
			  : GET_MODE (SET_DEST (set)) == GET_MODE (note)))
		    {
			 ...
			 be deleted or recognized by try_combine.  */
		      ...
		      SET_SRC (set) = note;
		      /* Try combining three insns.  */
		      for (nextlinks = LOG_LINKS (insn); nextlinks;
			   nextlinks = XEXP (nextlinks, 1))
			{
			  next = try_combine (insn, temp,
					      (nextlinks == links
					       ? NULL_RTX
					       : XEXP (nextlinks, 0)),
					      &new_direct_jump_p);
  ...
  switch (code)
	...
	return simplify_gen_unary (GET_CODE (XEXP (x, 0)), mode,
	 ...
	 (truncate:A X).  */
	  ...
	  && subreg_lowpart_p (XEXP (x, 0)))
	return simplify_gen_unary
	  (TRUNCATE, mode, XEXP (SUBREG_REG (XEXP (x, 0)), 0),
	   GET_MODE (XEXP (SUBREG_REG (XEXP (x, 0)), 0)));

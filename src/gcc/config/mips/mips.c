...
static int mips_adjust_priority (rtx, int);
...
static void octeon_select_section (tree, int, unsigned HOST_WIDE_INT)
				  ATTRIBUTE_UNUSED;
...
const struct mips_cpu_info mips_cpu_info_table[] = {
  ...
  /* A generic MIPS64r2 CPU.  */
  { "mips64r2", PROCESSOR_MIPS64R2_CPU, 65 },
...
mips_emit_scc (enum rtx_code code, rtx target)
{
  ...
  if (code == EQ || code == NE)
    {
      if (ISA_HAS_SEQ_SNE && reg_imm10_operand (cmp_operands[1], VOIDmode))
...
gen_conditional_branch (rtx *operands, enum rtx_code code)
{
  ...
  /* For now just remove the mode from integral comparisons.  */
  mode = GET_MODE (op0) == CCmode ? CCmode : VOIDmode;
...
/* The canonical form of mask-low-and-left-shift operation is
...
{
  return exact_log2 ((INTVAL (mask) >> INTVAL (shift)) + 1);
}

/* Check if MASK and SHIFT are valid mask_low_and_shift operation if
   MAXLEN is the maxium length of consecutive bits that can make up
   MASK.  See mask_low_and_shift_len for the actual definition.  */

int
mask_low_and_shift_p (rtx mask, rtx shift, int maxlen)
{
  int len = mask_low_and_shift_len (mask, shift);

  return 0 <= len && len <= maxlen;
}
...
override_options (void)
{
  ...
  if (mips_arch == PROCESSOR_OCTEON && TARGET_HARD_FLOAT)
    error ("-mhard-float is not allowed in Octeon.");
  ...
  /* Make sure that the ISA supports TARGET_PAIRED_SINGLE_FLOAT when it is
     enabled.  */
  if (TARGET_PAIRED_SINGLE_FLOAT && !ISA_MIPS64 && !ISA_MIPS64R2)
...
print_operand (FILE *file, rtx op, int letter)
{
  ...
  if (letter == 'C')
    ...
    switch (code)
      {
      case EQ:	fputs ("bit0",  file); break;
      case NE:	fputs ("bit1",  file); break;
      default:
	fatal_insn ("PRINT_OPERAND, invalid insn for %%C", op);	
      }
    ...
    switch (code)
      {
      case EQ:	fputs ("bit1",  file); break;
      case NE:	fputs ("bit0",  file); break;
      default:
	fatal_insn ("PRINT_OPERAND, invalid insn for %%C", op);	
      }
...
/* Output assembly instructions to peform a conditional branch.
   ...
   BIT_P is nonzero if this is a branch based on whether a bit
   specified by the third contant operand is set or clear (depending
   on INVERTED_P) in the second operand.
...
mips_output_conditional_branch (rtx insn, rtx *operands, int two_operands_p,
				int float_p, int inverted_p, int bit_p,
				int length)
{
  ...
  /* A string to use in the assembly output to represent the second
     ...
     operand.  */
  const char *op2 = (bit_p ? ",%3" : two_operands_p ? ",%z3" : ",%.");
  /* The operand-printing string for the comparison.  */
  const char *const comp = (float_p ? "%F0" : bit_p ? "%G0" : "%C0");
  /* The operand-printing string for the inverted comparison.  */
  const char *const inverted_comp = (float_p ? "%W0" : bit_p ? "%H0" : "%N0");
...
octeon_select_section (tree decl, int reloc, unsigned HOST_WIDE_INT align)
{
  if (decl && TREE_CODE (decl) == VAR_DECL 
      && lookup_attribute ("cvmx_shared", DECL_ATTRIBUTES (decl)))
    {
      ...
      if (sname)
	{
	  named_section_real (sname, flags, decl);
	  return;
	}
    }
...
octeon_output_shared_variable (FILE *stream, tree decl, const char *name,
                               unsigned HOST_WIDE_INT size, int align)
{
  named_section_flags (".cvmx_shared_bss", CVMX_SHARED_BSS_FLAGS);
  ASM_OUTPUT_ALIGN (stream, floor_log2 (align / BITS_PER_UNIT));
  ASM_DECLARE_OBJECT_NAME (stream, name, decl);
  ASM_OUTPUT_SKIP (stream, size != 0 ? size : 1);
}

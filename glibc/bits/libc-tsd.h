...
# else	/* OCTEON */
/* Provide both thread-local and global version so that we can use the
   fastest depending on the threading model.  */
#  define __libc_tsd_define(CLASS, KEY)				\
  CLASS __thread void *__libc_tsd_##KEY attribute_tls_model_ie;	\
  CLASS void *__libc_tsd_##KEY##_no_tls;
/* Note that we are not casting __thread away here.  __thread is a
   storage-class specifier so its address is no longer __thread
   "qualified".  */
#  define __libc_tsd_address(KEY)				\
  ({								\
    void **result = &__libc_tsd_##KEY##_no_tls;			\
    if (__pthread_mutex_lock)					\
      result =  &__libc_tsd_##KEY;				\
    result;							\
  })
...
#  define __libc_tsd_set(KEY, VALUE)		\
  ({						\
    if (__pthread_mutex_lock)			\
      __libc_tsd_##KEY = (VALUE);		\
    else					\
      __libc_tsd_##KEY##_no_tls = (VALUE);	\
  })
# endif	 /* OCTEON */
#else

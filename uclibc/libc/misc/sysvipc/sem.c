...
#ifdef __OCTEON__
    /* Change the type to cover the whole argument register so that we can
       pass it to the kernel unchanged.  (The kernel expects a 32-bit union
       here in the upper 32 bits of the arg.) */
    long long __pad;
#else /* ! __OCTEON__ */
    void *__pad;
#endif
...
# ifdef __OCTEON__
static inline _syscall4(int, __semctl, int, semid, int, semnum, int, cmd, long long, arg);
# else /* ! __OCTEON__ */
...
#ifdef __NR_semctl
# ifdef __OCTEON__
    /* Don't pass __IPC_64 as kernel does not expect it. Glibc just
       forwards the parameters to the system.  */
